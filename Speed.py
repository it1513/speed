...
class Speed:
    """
    Usage:
       Speed(data directory, video file name, fps, pixel coordinates, 
    ...
    def inspect_dynamics(self):
        ...
        while(1):
            ...
            if idx.size != 0:
                ...
                speed = log[idx,2]*2.23694 # Estimated speed (km)
            ...
            cv2.putText(img, "cas: %.2f sec" % elapsed,(10*scale,127*scale), 
                        font, 0.5*scale, (255,255,255), 1*scale)
            cv2.putText(img, "pocet aut: %d" % count,(10*scale,147*scale), 
                        font, 0.5*scale, (255,255,255), 1*scale)
            cv2.putText(img, "rychlost: %.2f km" % speed,(10*scale,167*scale), 
                        font, 0.5*scale, (255,255,255), 1*scale)
            cv2.putText(img, "vzdalenost: %.2f m" % distance,(10*scale,187*scale), 
                        font, 0.5*scale, (255,255,255), 1*scale)
            img = img[200:-100,:]
            out.write(img)
